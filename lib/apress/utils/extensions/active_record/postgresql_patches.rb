...
ActiveRecord::ConnectionAdapters::PostgreSQLColumn.class_eval do
  ...
  def simplified_type_with_exceptions(field_type)
    case field_type
      # state types
      when /^(product_state|company_about_state|tsvector)$/
          :string
      else
        simplified_type_without_exceptions(field_type)
    end
  end
  alias_method_chain :simplified_type, :exceptions
